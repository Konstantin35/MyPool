    ...
    async.each(redisClients, function(client, callback) {
      ...
      client.client.multi(redisCommands).exec(function(err, replies) {
        if (err) {
          ...
          for (var item = 0; item < replies.length; item += commandsPerCoin) {
            let i = item
            ...
            daemon.cmd('getinfo', [], function (result) {
              if (result[0].error) {
                  logger.error('Could not dumpprivkey for %s , err = %s',  JSON.stringify(result[0].error));
                  cback();
                  return;
              }
              networkInfo[coinName] = result[0].response;
              daemon.cmd('getmininginfo', [], function (result) {
                ...
                networkInfo[coinName]['mining'] = result[0].response;
                
                
                // Coins
                if (networkInfo[coinName]['mining'].hasOwnProperty('netmhashps')) {
                   networkInfo[coinName]['mining'].hashrate = networkInfo[coinName]['mining'].netmhashps * 1000000;
                }
                
                 if (networkInfo[coinName]['mining'].hasOwnProperty('networkhashps')) {
                   networkInfo[coinName]['mining'].hashrate = networkInfo[coinName]['mining'].networkhashps;
                } 
                
                   //console.log(networkInfo[coinName]);            
                var netdiff=0;           
                if (networkInfo[coinName].hasOwnProperty('difficulty')) {
                  netdiff = networkInfo[coinName].difficulty.hasOwnProperty('proof-of-work') ? networkInfo[coinName]['difficulty']['proof-of-work'] : networkInfo[coinName]['difficulty']; 
                }
                
                allCoinStats[coinName] = {
                  name: coinName,
                  ...
                  poolStats: {
                    ...
                    networkBlocks: networkInfo[coinName].blocks || 0,
                    networkSols:  networkInfo[coinName].hasOwnProperty('mining') ? networkInfo[coinName]['mining'].hashrate : 0,
                    networkSolsString: _this.getReadableHashRateString( networkInfo[coinName].hasOwnProperty('mining') ? networkInfo[coinName]['mining'].hashrate : 0),
                    networkDiff:  netdiff ,
                    networkConnections: networkInfo[coinName].connections || 0,
                    networkVersion: networkInfo[coinName].version || "sync",
                    networkProtocolVersion:  networkInfo[coinName].protocolversion || 0,
                    networkMoneysupply:  Math.round(networkInfo[coinName].moneysupply) || 0,
                    myBalance:  Math.round(networkInfo[coinName].balance) || 0,
                    errors:  networkInfo[coinName].errors === '' ?  "Everything is good" : networkInfo[coinName].errors
                  },
                  blocks: {
                    pending: replies[i + 3],
